...
export default function MatchingInterface() {
  ...
  useEffect(() => {
    ...
    fetch("http://127.0.0.1:8000/api/maindata")  // Adjust the URL as needed
      .then(response => {
        if (!response.ok) {
      ...
      .then(data => setProfileData(data[0])) // Access the first element of the array
